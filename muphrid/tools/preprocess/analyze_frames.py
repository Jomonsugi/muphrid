...
This is the agent's equivalent of Siril's "Plot" tab. After registration, Siril writes per-frame metrics to the *calibrated* sequence's .seq
file as R-lines:
...
def analyze_frames(
    ...
    If has_registration_data is False, re-run siril_register with tuned findstar
    ...
    if not calibrated_sequence:
        raise ValueError("calibrated_sequence not found in state. Run calibrate and register first.")
    ...
    if not seq_path.exists():
        raise FileNotFoundError(
            ...
            f"Ensure calibration and registration completed."
        )

    # Artifact-based validation (mirrors calibrate / register):
    ...
    if seq_size == 0:
        raise RuntimeError(
            ...
            f"The upstream step that wrote this .seq path produced no frame "
            f"data. Identify which tool wrote paths.calibrated_sequence or "
            f"paths.registered_sequence and inspect its output."
        )
    ...
    if seq_data["n_frames"] == 0:
        ...
        raise RuntimeError(
            f"analyze_frames: Sequence file {seq_path} has no valid Siril header "
            f"(S-line with n_frames). The file is not a real Siril sequence — "
            f"either corrupted, truncated, or pointing at the wrong path. "
            f"First 400 chars:\n{head}"
        )
    ...
    # is the expected "re-run siril_register with tuned findstar" path, not a hard
    # fail — the tool completes successfully and the agent decides.
    if not summary["has_registration_data"]:
        summary = {
            **summary,
            "hint": (
                "No registration data in the .seq file — Siril's findstar produced "
                "no per-frame measurements. The .seq header parsed fine "
                f"({summary['frame_count']} frames), so the sequence itself is "
                "valid; only the per-frame star table is missing. "
                "siril_register's threshold, radius, relax, and minstars parameters "
                "control findstar sensitivity for star-sparse fields."
            ),
        }
