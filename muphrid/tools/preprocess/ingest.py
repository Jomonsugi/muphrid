...
Sensor characterization:
  ...
  values are stored in AcquisitionMeta so downstream tools (build_masters)
...
from muphrid.equipment import resolve_focal_length, resolve_pixel_size
from muphrid.graph.state import AcquisitionMeta, Dataset, FileInventory
from muphrid.tools._sensor import sensor_info_from_tags
...
def _cross_validate_calibration(
    ...
    Warnings are informational — they do not halt ingestion. build_masters may escalate
        ...
        if sample:
            ...
            if sample.black_level == 0 and light_meta.get("black_level", 0) != 0:
                warnings.append(
                    ...
                    "flat quality thresholds in build_masters may fall back to safe defaults."
                )
...
def _ingest_raw(
    ...
    if not buckets["darks"]:
        warnings.append(
            ...
            "using bias-only calibration in calibrate."
        )
...
def ingest_dataset(
    ...
    These values enable build_masters to apply sensor-relative HITL thresholds for flat
