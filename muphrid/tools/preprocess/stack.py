...
class SirilStackInput(BaseModel):
    ...
    rejection_method: str = Field(
        description=(
            ...
            "Must be chosen based on accepted frame count from select_frames:\n"
    ...
    maximize: bool = Field(
        ...
        description=(
            ...
            "Larger canvas with borders — use with framing='max' in siril_register."
        ),
    )
    upscale: bool = Field(
        default=False,
    ...
    total_frames_hint: int | None = Field(
        default=None,
        description=(
            "Total registered frames from analyze_frames summary.frame_count. "
...
def _build_selection_commands(
    ...
    for filename in accepted_frames:
        ...
        if position is None:
            ...
            logger.warning(
                "siril_stack: accepted frame %r not found in %s — registration "
                "may have dropped it.",
                filename, seq_name,
            )
            continue
        ...
        if position < 1 or position > upper:
            skipped.append(filename)
            logger.warning(
                "siril_stack: position %d for frame %r out of range [1, %d] "
                "in %s — skipping.",
...
def siril_stack(
    ...
    from select_frames, then stacks with the specified method.
    ...
    if not registered_sequence:
        raise ValueError("registered_sequence not found in state. Run siril_register first.")
    ...
    if len(selection_cmds) == 1: # only the unselect survived
        raise RuntimeError(
            f"siril_stack: none of the {len(accepted_frames)} accepted frame keys "
            f"map to positions in {registered_sequence}.seq "
            ...
            f"I-lines). The accepted_frames list and the registered sequence are "
            f"inconsistent — either the registered .seq contains no included "
            f"frames, or the accepted_frames list was produced against a different "
            f"sequence. Skipped sample: {skipped_frames[:5]}"
        )
