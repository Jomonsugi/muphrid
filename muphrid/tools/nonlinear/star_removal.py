...
Input requirements:
  - Must be a stretched (non-linear) image (post stretch_image).
...
def star_removal(
    ...
    if not color_detected:
        raise RuntimeError(
            ...
            "Possible causes:\n"
            " 1. The FITS input to star_removal is already mono (1-channel) — check stretch_image output.\n"
            " 2. calibrate was run without debayer=True for a color camera.\n"
            " 3. star_removal was intentionally called on a luminance-only image (unusual).\n"
    ...
    # input. The documented contract is that star_removal runs post-stretch, so the
    ...
    # missing image_space means state is incomplete; refuse rather than
    # guess. See Metadata.image_space.
