...
def stretch_image(
    ...
    if fits_has_nan(img_path):
        raise ValueError(
            f"stretch_image requires finite pixel values. paths.current_image "
            f"({img_path.name}) contains NaN or Inf, which would produce an all-white "
            f"or corrupted stretch. Inspect the FITS for invalid pixels and identify "
            f"the upstream step that introduced them."
        )
