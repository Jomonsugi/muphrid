...
def saturation_adjust(
    ...
    if fits_nlayers(img_path) < 3:
        raise ValueError(
            f"saturation_adjust requires a 3-channel RGB image. "
            f"paths.current_image ({img_path.name}) is monochrome, so saturation "
            f"adjustment is a no-op. Inspect the channel count of "
            f"paths.current_image and identify the upstream step that produced it."
        )
