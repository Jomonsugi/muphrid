...
to any tool that performs plate solving (color_calibrate, plate_solve).

This is a mandatory early step in every pipeline run. The target name is required
input — the pipeline cannot proceed to plate-dependent operations (color_calibrate, plate_solve) without
...
Call this immediately after ingest_dataset. Store ra/dec in agent state and pass to
color_calibrate and plate_solve.
...
def resolve_target(
    ...
    for downstream plate solving tools (color_calibrate, plate_solve).
