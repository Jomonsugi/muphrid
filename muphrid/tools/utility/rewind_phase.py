...
# ── Phase ordering (mirrors advance_phase) ────────────────────────────────
...
def _get_rewind_reasoning(messages: list, tool_call_id: str | None) -> str:
    ...
    rewind_phase call. Mirrors advance_phase's _get_advance_reasoning so the agent's
...
def rewind_phase(
    ...
    if working_dir:
        ...
        try:
            from muphrid.tools.utility.advance_phase import (
