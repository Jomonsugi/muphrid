...
def enhance_star_color(
    ...
    if not (starless_is_color and mask_is_color):
        raise ValueError(
            "enhance_star_color requires 3-channel RGB starless and star_mask. "
            f"paths.starless_image is {'color' if starless_is_color else 'mono'}, "
            f"paths.star_mask is {'color' if mask_is_color else 'mono'}. "
            "Inspect the channel count of each artifact and identify the upstream "
            "step that produced the mono one."
        )
