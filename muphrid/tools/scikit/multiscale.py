...
Standard recipes (from spec §5 multiscale_process):
  Sharpening (on starless image with nebula luminance mask from create_mask):
...
class MultiscaleProcessInput(BaseModel):
    ...
    scale_operations: list[ScaleOperation] = Field(
        description=(
            ...
            "See spec §5 multiscale_process for standard sharpening and denoising recipes."
        )
    )
...
def multiscale_process(
    ...
    Set use_latest_mask=True to apply the most recent mask from create_mask.
    ...
    # pm, pixel_math) see NAXIS=2 — consistent with how Siril and create_mask save mono images.
