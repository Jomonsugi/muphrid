...
iteratively adjust thresholds and re-evaluate.
...
2. Optionally dilate the mask by `protect_core_radius` pixels to exclude the
   innermost star center from erosion (preserves star color).
...
- `protect_core_radius > 0`: applies `skimage.morphology.dilation` to create
  an exclusion zone around each star peak. Use when stars contain important
